...
IF EXIST SPHYSICS_SWE_gen_2D.exe (
  ...
  REM SPHYSICS_SWE_gen_2D.exe <Case_tsunami.txt > Case_tsunami.out
  SPHYSICS_SWE_gen_2D.exe <Case_Whelan.txt > Case_Whelan.out
  REM -- To use debugging in ftn95, use following line instead (remove REM, and comment out above line with REM)
  REM sdbg SPHYSICSgen_2D.exe <Case_tsunami.txt > Case_tsunami.out
